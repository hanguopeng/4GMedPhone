...
function showHis(){
  ...
  common.post({
    ...
    data:JSON.stringify({
      patientId: person.id,
      limit: -1,
      templateList:[{"templateCode":"gczqtzs","templateVersion":1}]
    }),
    success:function(ret){
...
function openDetailWin(id){
  api.openWin({
      name: 'win_gczqtzs_detail',
      url: './win_gczqtzs_detail.html',
      pageParam: {
          data: id
      }
  });
